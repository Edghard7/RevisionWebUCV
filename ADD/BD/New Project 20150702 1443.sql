...
INSERT INTO `persona` (`id_persona`,`id_tipo_persona`,`apellidos`,`nombres`,`correo`,`estado`) VALUES 
 ...
 (23,1,'OSORES PALACIOS','JUAN MANUEL','',1),
 (24,1,'____________','------------------','----------------',1),
...
INSERT INTO `usuario` (`id_usuario`,`id_persona`,`nombre`,`clave`,`fecha_registro`,`estado`) VALUES 
 ...
 (13,48,'rzapata48','e10adc3949ba59abbe56e057f20f883e','2015-07-01 11:57:45',1);
...
--
-- Definition of procedure `sp_funciones_persona`
--

DROP PROCEDURE IF EXISTS `sp_funciones_persona`;

DELIMITER $$

/*!50003 SET @TEMP_SQL_MODE=@@SQL_MODE, SQL_MODE='NO_ENGINE_SUBSTITUTION' */ $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_funciones_persona`(
in id_per int
)
BEGIN
...
FROM persona_funcion p
inner join persona per on p.id_persona = per.id_persona
inner join funcion f on p.id_funcion = f.id_funcion
where p.id_persona = id_per and p.estado = 1;
...
select p.*,u.nombre as usuario,t.nombre as tipo_persona, e.descripcion as estado_d from persona p
...
-- reemplaza tildes
    ...
    VALUES(NULL,@persona,@usuario,md5('123456'),now(),DEFAULT);
...
--
-- Definition of procedure `sp_REPORTE_lineas_x_escuela`
--

DROP PROCEDURE IF EXISTS `sp_REPORTE_lineas_x_escuela`;

DELIMITER $$

/*!50003 SET @TEMP_SQL_MODE=@@SQL_MODE, SQL_MODE='NO_ENGINE_SUBSTITUTION' */ $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_REPORTE_lineas_x_escuela`(
in escuela int
)
BEGIN


select l.nombre as linea,count(l.id_linea) as nro from linea_investigacion l
left join escuela es on es.id_escuela = l.id_escuela
left join tesis t on t.id_linea = l.id_linea
...
--
-- Definition of procedure `sp_REPORTE_observaciones_x_tesis`
--

DROP PROCEDURE IF EXISTS `sp_REPORTE_observaciones_x_tesis`;

DELIMITER $$

/*!50003 SET @TEMP_SQL_MODE=@@SQL_MODE, SQL_MODE='NO_ENGINE_SUBSTITUTION' */ $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_REPORTE_observaciones_x_tesis`(
in tesis int
)
BEGIN


SELECT t.titulo,tr.fecha_recepcion,o.*,e.descripcion as estado_des,concat(ifnull(de.capitulo,''),' ',de.nombre) as esquema
FROM tesis t
inner join tesis_recepcion tr on t.id_tesis = tr.id_tesis
inner join asignacion a on tr.id_tesis_recepcion = a.id_tesis_recepcion
inner join revision r on a.id_asignacion = r.id_asignacion
inner join observacion o on r.id_revision = o.id_revision
inner join detalle_esquema de on o.id_detalle_esquema = de.id_detalle

inner join estado e on o.estado = e.id_estado

where t.id_tesis = tesis;
...
--
-- Definition of procedure `sp_REPORTE_tesis_x_escuela`
--

DROP PROCEDURE IF EXISTS `sp_REPORTE_tesis_x_escuela`;

DELIMITER $$

/*!50003 SET @TEMP_SQL_MODE=@@SQL_MODE, SQL_MODE='NO_ENGINE_SUBSTITUTION' */ $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_REPORTE_tesis_x_escuela`()
BEGIN

SELECT es.nombre as escuela,count(t.id_tesis) as nro_tesis
FROM tesis t
inner join linea_investigacion l on t.id_linea = l.id_linea
inner join escuela es on l.id_escuela = es.id_escuela
group by es.id_escuela
order by es.nombre;
